...
class Infiltration(object):
    ...
    def dynamic(self):
        ...
        ToStore = np.zeros((self.var.nFarm, self.var.nCrop, self.var.domain.nxy))
        RunoffIni = np.zeros((self.var.nFarm, self.var.nCrop, self.var.domain.nxy))
        ...
        k_sat_top = self.var.k_sat_comp[:,:,0,:]
        cond1 = (self.var.Bunds == 1) & (self.var.zBund > 0.001)
        cond11 = (cond1 & (InflTot > 0))
        cond111 = (cond11 & (InflTot > k_sat_top))
        ToStore[cond111] = k_sat_top[cond111]
        self.var.SurfaceStorage[cond111] = (InflTot - k_sat_top)[cond111]
        ...
        cond21 = (cond2 & (self.var.Infl > k_sat_top))
        ToStore[cond21] = k_sat_top[cond21]
        RunoffIni[cond21] = (self.var.Infl - k_sat_top[0,:])[cond21]
        RunoffIni[cond21] = (self.var.Infl - k_sat_top)[cond21]
        # RunoffIni[cond21] = (self.var.Infl - k_sat_top[0,:])[cond21]
        ...
        # Initialize counters
        comp = 0
        Runoff = np.zeros((self.var.nFarm, self.var.nCrop, self.var.domain.nxy))
        cond3_ini = (ToStore > 0)
        ...
        while (np.any(ToStore > 0) & (comp < self.var.nComp)):
            ...
            dthdtS = self.var.tau_comp[:,:,comp,:] * (self.var.th_sat_comp[:,:,comp,:] - self.var.th_fc_comp[:,:,comp,:])
            factor = self.var.k_sat_comp[:,:,comp,:] / (dthdtS * 1000 * self.var.dz[comp])
            dthdt0 = ToStore / (1000 * self.var.dz[comp])

            # Initialize water content for current layer
            theta0 = np.zeros((self.var.nFarm, self.var.nCrop, self.var.domain.nxy))
            ...
            A = (1 + ((dthdt0 * (np.exp(self.var.th_sat_comp[:,:,comp,:] - self.var.th_fc_comp[:,:,comp,:]) - 1)) / (self.var.tau_comp[:,:,comp,:] * (self.var.th_sat_comp[:,:,comp,:] - self.var.th_fc_comp[:,:,comp,:]))))
            theta0[cond312] = (self.var.th_fc_comp[:,:,comp,:] + np.log(A))[cond312]

            # Limit thX to between saturation and field capacity
            cond313 = (cond31 & (theta0 > self.var.th_sat_comp[:,:,comp,:]))
            theta0[cond313] = self.var.th_sat_comp[:,:,comp,:][cond313]
            ...
            theta0[cond32] = self.var.th_sat_comp[:,:,comp,:][cond32]
            ...
            cond33 = (cond3 & (drainage > self.var.k_sat_comp[:,:,comp,:]))
            drainmax[cond33] = (self.var.k_sat_comp[:,:,comp,:] - self.var.FluxOut[:,:,comp,:])[cond33]
            ...
            while (np.any(cond3 & (excess > 0)) & (precomp != 0)):
                ...
                cond351 = (cond35 & (thnew[:,:,precomp,:] > self.var.th_sat_comp[:,:,precomp,:]))
                excess[cond351] = ((thnew[:,:,precomp,:] - self.var.th_sat_comp[:,:,precomp,:]) * 1000 * self.var.dz[precomp])[cond351]
                thnew[:,:,precomp,:][cond351] = self.var.th_sat_comp[:,:,precomp,:][cond351]
