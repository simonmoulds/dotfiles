...
class SoilEvaporation(object):
    ...
    def dynamic_fortran(self):
        ...
        self.var.EsAct = np.zeros(
            (self.var.nFarm, self.var.nCrop, self.var.domain.nxy))
        prec = np.broadcast_to(self.var.model.prec.values,
                               (self.var.nFarm, self.var.nCrop, self.var.domain.nxy))
        etref = np.broadcast_to(self.var.model.etref.values,
                                (self.var.nFarm, self.var.nCrop, self.var.domain.nxy))
        EvapTimeSteps = 20
    ...
    def dynamic_numpy(self):
        ...
        # prec = np.broadcast_to(self.var.precipitation[None,None,:], (self.var.nFarm, self.var.nCrop, self.var.domain.nxy))
        et0 = self.var.model.etref.values.copy()
        prec = self.var.model.prec.values.copy()
        # et0 = self.var.weather.referencePotET.copy()
        # prec = self.var.weather.precipitation.copy()
        # et0 = self.var.referencePotET[None,:] * np.ones((self.var.nCrop))[:,None]
        # prec = self.var.precipitation[None,:] * np.ones((self.var.nCrop))[:,None]

        # Prepare stage 2 evaporation (REW gone), if day one of simulation
        cond1 = (self.var.model.time.timestep == 1)
        if self.var.model.time.timestep == 1:
        ...
        cond2 = ((prec > 0) | np.any(
            ((self.var.Irr > 0) & (self.var.IrrMethod != 4)), axis=0))  # FIXME
        cond21 = (cond2 & (self.var.Infl > 0))
        self.var.Wsurf[cond21] = self.var.Infl[cond21]
        self.var.Wsurf[cond21] = np.clip(
            self.var.Wsurf, None, self.var.REW)[cond21]
        ...
        EsPotMul[cond111] = (
            EsPot * (1 - self.var.fMulch * (self.var.MulchPctGS / 100)))[cond111]
        cond112 = (cond11 & np.logical_not(self.var.GrowingSeasonIndex))
        EsPotMul[cond112] = (
            EsPot * (1 - self.var.fMulch * (self.var.MulchPctOS / 100)))[cond112]
        ...
        # TODO: more informative name for wet_surf
        EsPotIrr[cond12] = (EsPot * (self.var.WetSurf / 100))[cond12]
        # return EsPotIrr
        ...
        cond = ((self.var.SurfaceStorage > 0) & (
            self.var.SurfaceStorage <= EsPot))
        ...
        # TODO: add MASK argument to function?
        self.var.Wstage2[cond103] = Wstage2_tmp[cond103]
        ...
        if np.any(cond11):
            ...
            for jj in range(EvapTimeSteps):
                ...
                Wcheck = (self.var.fWrelExp * ((self.var.EvapZmax -
                                                self.var.EvapZ) / (self.var.EvapZmax - self.var.EvapZmin)))
                while np.any(cond111 & (Wrel < Wcheck) & (self.var.EvapZ < self.var.EvapZmax)):
                    cond1111 = (cond111 & (Wrel < Wcheck) & (
                        self.var.EvapZ < self.var.EvapZmax))
                ...
                Kr = ((np.exp(self.var.fevap * Wrel) - 1) /
                      (np.exp(self.var.fevap) - 1))
    ...
    def evap_layer_water_content(self):
        ...
        evapz_comp = self.var.EvapZ[:, :, None, :] * \
            np.ones((self.var.nComp))[None, None, :, None]
        comp_sto = (np.round((self.var.dz_sum_xy - self.var.dz_xy)
                             * 1000) < np.round(evapz_comp * 1000))
        ...
        # sum along comp dim
        Wevap_Act = np.sum(
            (factor * 1000 * self.var.th * self.var.dz_xy), axis=2)
        self.var.Wevap_Act = np.clip(Wevap_Act, 0, None)
        self.var.Wevap_Sat = np.sum(
            (factor * 1000 * self.var.th_sat_comp * self.var.dz_xy), axis=2)
        self.var.Wevap_Fc = np.sum(
            (factor * 1000 * self.var.th_fc_comp * self.var.dz_xy), axis=2)
        self.var.Wevap_Wp = np.sum(
            (factor * 1000 * self.var.th_wilt_comp * self.var.dz_xy), axis=2)
        self.var.Wevap_Dry = np.sum(
            (factor * 1000 * self.var.th_dry_comp * self.var.dz_xy), axis=2)

    def prepare_stage_two_evaporation(self):
        self.evap_layer_water_content()
        Wstage2 = (
            (self.var.Wevap_Act - (self.var.Wevap_Fc - self.var.REW))
            / (self.var.Wevap_Sat - (self.var.Wevap_Fc - self.var.REW)))
    ...
    def potential_soil_evaporation_rate(self, tAdj):
        ...
        EsPotMax = (self.var.Kex * et0 *
                    (1 - self.var.CCxW * (self.var.fwcc / 100)))
        EsPot[self.var.GrowingSeasonIndex] = (
            self.var.Kex * (1 - self.var.CCadj) * et0)[self.var.GrowingSeasonIndex]

        # Adjust potential soil evaporation for effects of withered canopy
        cond3 = (self.var.GrowingSeasonIndex & (
            tAdj > self.var.Senescence) & (self.var.CCxAct > 0))
        ...
        mult[cond312] = np.divide(mult_divd, mult_divs, out=np.zeros_like(
            mult_divs), where=mult_divs != 0)[cond312]

        EsPot[cond3] = (EsPot * (1 - self.var.CCxAct *
                                 (self.var.fwcc / 100) * mult))[cond3]
        CCxActAdj = ((1.72 * self.var.CCxAct) +
                     (self.var.CCxAct ** 2) - 0.3 * (self.var.CCxAct ** 3))
        EsPotMin = np.zeros(
            (self.var.nFarm, self.var.nCrop, self.var.domain.nxy))
    ...
    def adjust_potential_soil_evaporation_for_irrigation(self, EsPot):
        ...
        cond12 = (cond1 & np.logical_not(cond11))
        # TODO: more informative name for wet_surf
        EsPotIrr[cond12] = (EsPot * (wet_surf / 100))[cond12]
        return EsPotIrr
    ...
    def extract_water(self, ToExtract, ToExtractStg):
        ...
        evapzmin_comp = self.var.EvapZmin[:, :, None, :] * \
            np.ones((self.var.nComp))[None, None, :, None]  # metres
        comp_sto = (np.round((self.var.dz_sum_xy - self.var.dz_xy)
                             * 1000) < np.round(evapzmin_comp * 1000))  # fraction
        factor = 1 - ((self.var.dz_sum_xy - evapzmin_comp) /
                      self.var.dz_xy)  # fraction
        factor = np.clip(factor, 0, 1) * comp_sto

        comp_sto = np.sum(comp_sto, axis=2)  # sum along comp dim
        comp = 0
        while np.any((comp < comp_sto) & (ToExtractStg > 0) & (ToExtract > 0)):

            cond101 = ((comp < comp_sto) & (
                ToExtractStg > 0) & (ToExtract > 0))

            # Water available in compartment for extraction (mm)
            Wdry = 1000 * self.var.th_dry_comp[:, :,
                                               comp, :] * self.var.dz_xy[:, :, comp, :]
            W = 1000 * self.var.th[:, :, comp, :] * \
                self.var.dz_xy[:, :, comp, :]
            ...
            W[cond1012] -= AvW[cond1012]                        # AvW is in mm
            # ToExtract is in m
            ...
            self.var.th[:, :, comp, :][cond101] = (
                W / (1000 * self.var.dz_xy[:, :, comp, :]))[cond101]
            comp += 1
    ...
    def relative_depletion(self):
        ...
        self.evap_layer_water_content()
        # Get water storage (mm) at start of stage 2 evaporation
        Wupper = (self.var.Wstage2 * (self.var.Wevap_Sat - (self.var.Wevap_Fc -
                                                            self.var.REW)) + (self.var.Wevap_Fc - self.var.REW))
        ...
        Wrel = np.divide(Wrel_divd, Wrel_divs, out=np.zeros_like(
            Wrel_divs), where=Wrel_divs != 0)
