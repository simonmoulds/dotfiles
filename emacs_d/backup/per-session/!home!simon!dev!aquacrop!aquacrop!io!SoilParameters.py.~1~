...
class SoilParameters(object):
    def __init__(self, model):# , config_section_name):
        self.model = model
        self.model.adjustReadilyAvailableWater = self.model.config.SOIL_PARAMETERS['adjustReadilyAvailableWater']
    ...
    def compute_curve_number_limits(self):
        ...
        self.model.CNbot = np.round(1.4 * (np.exp(-14 * np.log(10))) + (0.507 * self.model.CN) - (0.00374 * self.model.CN ** 2) + (0.0000867 * self.model.CN ** 3))
        self.model.CNtop = np.round(5.6 * (np.exp(-14 * np.log(10))) + (2.33 * self.model.CN) - (0.0209 * self.model.CN ** 2) + (0.000076 * self.model.CN ** 3))

    def compute_readily_available_water(self):
        self.model.REW = (np.round((1000 * (self.model.th_fc[...,0,:] - self.model.th_dry[...,0,:]) * self.model.EvapZsurf)))
    ...
    def compute_weighting_factor_for_cn_adjustment(self):
        ...
        zcn = np.broadcast_to(self.model.zCN[...,None,:], (self.model.nFarm, self.model.nCrop, self.model.nComp, self.model.domain.nxy))
        ...
        self.model.weighting_factor_for_cn_adjustment = np.clip((wx - xx), 0, 1)
    ...
    def adjust_zgerm(self):
        ...
        zgerm[zgerm > np.sum(self.model.dz, axis=0)] = np.sum(self.model.dz, axis=0)
